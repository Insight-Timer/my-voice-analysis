...
class MyVoiceAnalysis:
    ...
    def total(self, soundfile):
        ...
        objects = run_file(self.praat_file, -20, 2, 0.3, "yes",
                           soundfile, soundfile_dir, 80, 400, 0.01, capture_output=True)
        ...
        return {
            "number_ of_syllables": float(z5[0, 0]),
            "number_of_pauses": float(z5[1, 0]),
            "rate_of_speech": float(z5[2, 0]),
            "articulation_rate": float(z5[3, 0]),
            "speaking_duration": float(z5[4, 0]),
            "original_duration": float(z5[5, 0]),
            "balance": float(z5[6, 0]),
